...
// File-cache configuration (stored relative to working dir)
const CACHE_DIR = path.join(process.cwd(), '.cache');
const BGP_CACHE_FILE = path.join(CACHE_DIR, 'bgp-table.jsonl');
const ASNS_CACHE_FILE = path.join(CACHE_DIR, 'asns.csv');
...
async function downloadBGPTable(): Promise<string | null> {
  ...
  if (bgpTableCache !== null && now - bgpTableCacheTime < CACHE_TTL) {
    console.log('  Using in-memory cached BGP table data');
    return bgpTableCache;
  }

  // Try file-based cache: use file's mtime to determine freshness
  try {
    const stat = await fs.promises.stat(BGP_CACHE_FILE);
    const mtime = stat.mtimeMs;
    if (now - mtime < CACHE_TTL) {
      const data = await fs.promises.readFile(BGP_CACHE_FILE, { encoding: 'utf8' });
      bgpTableCache = data;
      bgpTableCacheTime = now;
      console.log('  Using cached BGP table from file');
      return bgpTableCache;
    }
  } catch (e) {
    // File not present or unreadable - will attempt download
  }
  ...
  try {
    ...
    try {
      ...
      await fs.promises.writeFile(BGP_CACHE_FILE, response.data, { encoding: 'utf8' });
      console.log('  Wrote BGP table to file cache');
    } catch (e) {
      console.warn('  Could not write BGP table to file cache:', e);
    }
...
async function downloadASNsData(): Promise<Map<number, ASNMetadata> | null> {
  ...
  if (asnsCache !== null && now - asnsCacheTime < ASNS_CACHE_TTL) {
    console.log('  Using in-memory cached ASNs data');
    return asnsCache;
  }

  // Try file-based cache: use file's mtime to determine freshness
  try {
    const stat = await fs.promises.stat(ASNS_CACHE_FILE);
    const mtime = stat.mtimeMs;
    if (now - mtime < ASNS_CACHE_TTL) {
      const data = await fs.promises.readFile(ASNS_CACHE_FILE, { encoding: 'utf8' });
      // Parse cached CSV
      ...
      asnsCache = asnsMap;
      asnsCacheTime = now;
      console.log('  Using cached ASNs CSV from file');
      return asnsCache;
    }
  } catch (e) {
    // File not present or unreadable - will attempt download
  ...
  try {
    ...
    // Persist CSV to file cache
    try {
      await fs.promises.mkdir(CACHE_DIR, { recursive: true });
      await fs.promises.writeFile(ASNS_CACHE_FILE, response.data, { encoding: 'utf8' });
      console.log('  Wrote ASNs CSV to file cache');
    } catch (e) {
      console.warn('  Could not write ASNs CSV to file cache:', e);
    }
